...
| :---------------------- | :--------------- | :------------------- | :----------------------- | :------------------- | :--------------- |
| Chain of Responsibility | イベント処理     | ログ出力フィルター   | 例外処理                 | メッセージ処理       | コマンド実行     |
| Command	キュー操作      | アンドゥ/        | リドゥ               | マクロ操作               | ジョブスケジューラ   | メニューコマンド |
| Interpreter             | SQLパーサー      | 正規表現処理         | コード解析               | 式計算               | マークアップ解析 |
| Iterator                | 配列走査         | ツリー走査           | グラフ走査               | ファイルリスト       | ページング処理   |
| Mediator                | チャットアプリ   | ゲームマッチング     | ウィンドウコンポーネント | ワークフローシステム | グラフィックUI   |
| Memento	ゲームセーブ    | エディタ状態     | アンドゥ/            | リドゥ                   | スナップショット     | 設定復元         |
| Observer                | イベントリスナ   | データバインディング | ログ監視                 | アプリ通知           | プログレスバー   |
| State                   | ゲームステート   | 通信プロトコル       | ワークフロー             | ツールスイッチ       | プリントジョブ   |
| Strategy                | 圧縮アルゴリズム | 検索アルゴリズム     | レンダリング方法         | データソート         | パスワード暗号化 |
| Template Method         | コンパイラ       | データパーサー       | レポート生成             | ウィザード           | テキスト生成     |
| Visitor                 | シーングラフ     | オブジェクト検証     | ツリー操作               | コード生成           | テキスト解析     |
